...
from formations.models import GuestEmail
...
class BillingProfile(models.Model):
    ...
    def charge(self, reservation_obj, card=None):
        return Charge.objects.do(self, reservation_obj, card)
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, reservation_obj, card=None): # Charge.objects.do()
        card_obj = card
        if card_obj is None:
        ...
        c = stripe.Charge.create(
              amount = int(reservation_obj.total * 100), # 39.19 --> 3919
              currency = "eur",
              customer =  billing_profile.customer_id,
              source = card_obj.stripe_id,
              metadata={"reservation_id":reservation_obj.reservation_id},
            )
